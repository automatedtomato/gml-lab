...
// ===== Requant kernels =====
__global__ void kernel_requantize_per_tensor(
...
__global__ void kernel_requantize_per_channel(
    const int32_t* __restrict__ input_int32,
    int8_t* __restrict__ output_int8,
...
torch::Tensor quant_linear(
    ...
    typename GemmBase::Arguments args(
        {m, n, k},
        {input.data_ptr<int8_t>(), k},  // stride = k
        {weight.data_ptr<int8_t>(), k},
        {bias_ptr, 0},
        {temp_output.data_ptr<int32_t>(), n},
        {1.0f, 1.0f}   // alpha=1, beta=1 (D = 1*(A*B) + 1*C)
    );
    ...
    if (is_per_channel) {
        kernel_requantize_per_channel<<<blocks, kDefaultThreads>>>(
            temp_output.data_ptr<int32_t>(),
            final_output.data_ptr<int8_t>(),
        ...
        kernel_requantize_per_tensor<<<blocks, kDefaultThreads>>>(
            temp_output.data_ptr<int32_t>(),
            final_output.data_ptr<int8_t>(),
            scale_val, output_zp, total_elements
        );
    }

    cudaError_t err = cudaGetLastError();
    TORCH_CHECK(err == cudaSuccess, "Requantize kernel failed: ", cudaGetErrorString(err));
